...
def postar_humidade(dado: TemperaturaEntrada):  # reutilizando o modelo com sensor/valor
    ...
    result = collection_humidade.insert_one(registro)
    registro["_id"] = str(result.inserted_id)
    return {"msg": "Humidade inserida com sucesso", "dados": registro}
...
def get_ultima_temperatura_cupula():
    ...
    if ultimo:
        ...
        return JSONResponse(content={"error": "Sem temperatura da cúpula encontrada"}, status_code=404)

        
@app.get("/ultima_humidade")
def get_ultima_humidade():
    ultimo = collection_humidade.find_one(sort=[("timestamp", -1)])
    if ultimo:
        return {
        ...
        return JSONResponse(content={"error": "Sem humidade encontrada"}, status_code=404)
